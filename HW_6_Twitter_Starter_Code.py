...
def construct_unique_key(baseurl, params):
    ...
    for k in params.keys():
        param_strings.append(f'{k}{connector}{params[k]}')
    param_strings.sort()
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    '''
    #TODO Implement function
    params = {'q':f'{hashtag}'} # e.g. q=%23superbowl
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    ----------
    tweet_data: dict
        Twitter data as a dictionary for a specific query (made with hashtag_to_ignore)
    ...
    hashtags_count = {} # hashtag:counts

    #count all hashtags
    for i,tweet in enumerate(tweet_data['statuses']):
        hashtags = tweet_data['statuses'][i]['entities']['hashtags']
        for j,hashtag in enumerate(hashtags):
            tag = hashtags[j]['text'].lower()
            if tag in hashtags_count.keys():
                hashtags_count[tag] += 1
            else:
                hashtags_count[tag] = 1

    # throw away the hashtag_to_ignore
    # print(hashtags_count)
    hashtag_to_ignore = hashtag_to_ignore.replace('#','')
    hashtags_count.pop(hashtag_to_ignore.lower())

    #find max
    max_count = max(hashtags_count.values())
    # print(max_count)
